...
class webStore ctx resolver repo token id host_id = 
  ...
  object (self)
    ...
    method private post_ready (time: string) =
      ...
      if code == 200 then begin
        Logs.info (fun m -> m "Notified MirageManager of being ready at %s" time);
        Lwt.return true
      end else begin
    ...
    method private get_state (time: string) =
      ...
      if code == 200 then begin
        Logs.info (fun m -> m "Got state: %s at %s" body_str time);
    ...
    method private post_state (time: string) (status: Control.Status.t) =
      ...
      let code = response |> Cohttp.Response.status |> Cohttp.Code.code_of_status in
      if code == 200 then begin
        Logs.info (fun m -> m "Wrote state to repo at %s" time);
        Lwt.return true
      end else begin
    ...
    method suspend (time: string) (status: Control.Status.t) =
    ...
    method init (time: string) (migration: bool) (steady: (unit -> bool Lwt.t)) =
